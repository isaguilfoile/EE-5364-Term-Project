    ...
    if (MSHR.next_fill_cycle <= current_core_cycle[fill_cpu]) {
		...
		cpu = 0;
            ...
            if (cache_type == IS_LLC) {
                ...
                update_replacement_state(fill_cpu, set, way, MSHR.entry[mshr_index].full_addr, MSHR.entry[mshr_index].ip, block[set][way].full_addr, MSHR.entry[mshr_index].type, 0, MSHR.entry[mshr_index].instr_id, block[set]);
